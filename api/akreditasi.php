...
if ($method === 'GET' && $action === 'periods') {
    $sql = 'SELECT year, month, paripurna, utama, madya, updated_at, NULL AS region
            FROM akreditasi_periods
            UNION ALL
            SELECT year, month, paripurna, utama, madya, updated_at, region
            FROM akreditasi_regions
            ORDER BY year DESC, month DESC, region IS NULL, region';
    $stmt = $pdo->query($sql);
    $rows = $stmt->fetchAll();
    respond($rows);
}
...
if ($method === 'GET') {
    ...
    if ($year > 0 && $month >= 1 && $month <= 12) {
        // If region specified, try region-specific record first
        if ($region !== '') {
            $stmt = $pdo->prepare('SELECT paripurna, utama, madya FROM akreditasi_regions WHERE year = ? AND month = ? AND region = ?');
            $stmt->execute([$year, $month, $region]);
            $row = $stmt->fetch();
            if ($row) {
                respond($row);
            }
        ...
        // then try period-specific global value
...
if ($method === 'PUT') {
    ...
    $bodyRegion = isset($data['region']) ? trim($data['region']) : '';
    if ($bodyRegion !== '') {
        $region = $bodyRegion;
    }
    ...
    if ($year > 0 && $month >= 1 && $month <= 12) {
        if ($region !== '') {
            // upsert region-specific record
            $stmt = $pdo->prepare('INSERT INTO akreditasi_regions (year, month, region, paripurna, utama, madya) VALUES (?,?,?,?,?,?)
              ON DUPLICATE KEY UPDATE paripurna = VALUES(paripurna), utama = VALUES(utama), madya = VALUES(madya)');
            $stmt->execute([$year, $month, $region, $paripurna, $utama, $madya]);
        } else {
            // upsert period-specific global record
        ...
        // fallback to global single-row akreditasi (provinsi)
