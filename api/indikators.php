...
if ($method === 'GET') {
    ...
    $sql = 'SELECT id, name, capaian, target, status, date FROM indikators';
...
if ($method === 'POST') {
    ...
    $stmt = $pdo->prepare('INSERT INTO indikators (name, capaian, target, status, date) VALUES (?,?,?,?,?)');
    $stmt->execute([$name, $capaian, $target, $status, $date]);
    $id = (int)$pdo->lastInsertId();
    respond(['id' => $id, 'name' => $name, 'capaian' => $capaian, 'target' => $target, 'status' => $status, 'date' => $date], 201);
}
...
if ($method === 'PUT') {
    ...
    if ($id > 0) {
        ...
        $stmt = $pdo->prepare('UPDATE indikators SET name=?, capaian=?, target=?, status=?, date=? WHERE id=?');
        $stmt->execute([$name, $capaian, $target, $status, $date, $id]);
        respond(['id' => $id, 'name' => $name, 'capaian' => $capaian, 'target' => $target, 'status' => $status, 'date' => $date]);
    } else {
