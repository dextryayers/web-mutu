...
export function AppProvider({ children }) {
  ...
  async function register(username, password, email, phone, role = 'member', nik, fullName, city, instansi, recaptchaToken, loginRecaptchaToken) {
    if (useApi) {
      await API.register(username, password, email, phone, role, nik, fullName, city, instansi, recaptchaToken)
  ...
  async function createIndicator(name = 'New indikator', capaian = 0, target = 100, date = null) {
    if (useApi) {
      const created = await API.createIndicator({ name, capaian, target, date })
      ...
      const row = { id, name, capaian, target, status, date }
      setData(prev => ({ ...prev, indikators: [...prev.indikators, row] }))
    }
  }

  async function updateIndicatorRow({ id, name, capaian, target, date = null, status }) {
    if (useApi) {
      const updated = await API.updateIndicator({ id, name, capaian, target, date, status })
      ...
      setData(prev => ({ ...prev, indikators: prev.indikators.map(i => (i.id === id ? { ...i, name, capaian, target, status: computedStatus, date } : i)) }))
    }
  }
  ...
  async function createAdminUser({ username, password, email, phone, nik }, recaptchaToken) {
    if (useApi) {
      return API.createAdminUser({ username, password, email, phone, nik }, recaptchaToken)
    }
    return null
  }
