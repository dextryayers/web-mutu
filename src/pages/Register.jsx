...
export default function Register() {
  ...
  async function submit(e) {
    ...
    if (role === 'admin') {
      if (!trimmedNik) {
        setError('NIK wajib diisi untuk admin')
        return
      }
      if (!/^\d{8,20}$/.test(trimmedNik)) {
    ...
    try {
      ...
      const ok = await register(
        ...
        phone, 
        role, 
        role === 'admin' ? trimmedNik : undefined,
        trimmedFullName,
    ...
    <div className="container mx-auto px-4 py-12">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
          <button
            type="button"
            onClick={() => { setRole('member'); setNik(''); setCity(''); setInstansi('') }}
        ...
        <div className="bg-white dark:bg-slate-800 rounded shadow p-6">
          ...
          <form onSubmit={submit} className="space-y-4">
            ...
            {role === 'admin' && (
              <>
                <div>
                  <label className="text-sm text-slate-700 dark:text-slate-300">NIK (Admin)</label>
                  <input
                    value={nik}
                    ...
                    required
                    className="mt-1 w-full border px-3 py-2 rounded dark:bg-slate-900 dark:text-slate-100 dark:border-slate-700"
                    placeholder="Masukkan 8-20 digit"
                  />
                  <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Hanya angka, 8-20 digit. Wajib untuk admin.</p>
                </div>
                <div>
