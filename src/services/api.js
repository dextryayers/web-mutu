...
async function parseJson(r) {
  ...
  if (!ct.includes('application/json')) {
    // Try to parse as JSON; if it fails, avoid surfacing raw HTML/error pages to the UI
    try {
...
export async function getAkreditasi({ year, month, region } = {}) {
  const params = [];
...
export async function updateAkreditasi(payload) {
  ...
  if (year) params.push(`year=${encodeURIComponent(year)}`);
  if (month) params.push(`month=${encodeURIComponent(month)}`);
  if (payload.region) params.push(`region=${encodeURIComponent(payload.region)}`);
  const qs = params.length ? `?${params.join('&')}` : '';
...
export async function createIndicator({ name, capaian = 0, target = 100, date = null, region = null }) {
  return request('indikators.php', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ name, capaian, target, date, region }),
  });
}

export async function updateIndicator({ id, name, capaian, target, date = null, status, region = null }) {
  const payload = { name, capaian, target, date, region };
...
// Auth
// Note: nik parameter is kept for backward compatibility with existing callers,
// but it is intentionally not used for persistence because the users table has no nik column.
export async function register(username, password, email, phone, role = 'member', nik, fullName, city, instansi, recaptchaToken, adminCode) {
  const payload = { username, password, email, phone, role, fullName };
  if (city) payload.city = city;
  if (instansi) payload.instansi = instansi;
  if (recaptchaToken) payload.recaptchaToken = recaptchaToken;
  if (role === 'admin' && adminCode) payload.adminCode = adminCode;
...
export async function createAdminUser({ username, password, email, phone, fullName, city, instansi }, recaptchaToken) {
  const payload = { username, password, email, phone, fullName, city, instansi };
  if (recaptchaToken) payload.recaptchaToken = recaptchaToken;
...
export async function updateProfile({ email, phone, password, photoUrl }) {
  const payload = {};
