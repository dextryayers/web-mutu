...
export default function ChartEditor() {
  ...
  async function save() {
    try {
      await saveAkreditasi({ ...local, year: year ? Number(year) : undefined, month: month ? Number(month) : undefined })
      // feedback
  ...
  function exportHistory() {
    ...
    const header = ['Periode', 'Paripurna', 'Utama', 'Madya', 'TerakhirDiubah']
      ...
      <div className="mt-8">
        ...
        <div className="mt-3 flex flex-col md:flex-row gap-3 md:items-end">
          <div className="flex gap-2">
            <select value={historyMonth} onChange={e => setHistoryMonth(e.target.value)} className="px-2 py-1 border rounded text-sm w-32 dark:bg-slate-900 dark:text-slate-100 dark:border-slate-700">
              <option value="">Semua Bulan</option>
              {Array.from({ length: 12 }, (_, i) => (
                <option key={i + 1} value={i + 1}>{monthNames[i]}</option>
              ))}
            </select>
            <select value={historyYear} onChange={e => setHistoryYear(e.target.value)} className="px-2 py-1 border rounded text-sm w-28 dark:bg-slate-900 dark:text-slate-100 dark:border-slate-700">
          ...
          <table className="min-w-full border text-sm">
            ...
            <tbody>
              {filteredHistory.length === 0 && (
                <tr>
                  <td colSpan="5" className="px-3 py-4 text-center text-slate-500 dark:text-slate-400">Belum ada riwayat tersimpan.</td>
                </tr>
              )}
              {filteredHistory.map(item => (
                <tr key={`${item.year}-${item.month}`} className="odd:bg-white even:bg-slate-50 dark:odd:bg-slate-800 dark:even:bg-slate-900">
