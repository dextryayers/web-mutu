...
export default function DataManager({ docsOnly = false }) {
  ...
  async function saveEdit() {
    try {
      await updateIndicatorRow({ id: editing, name: form.name, capaian: form.capaian, target: form.target, date: form.date, status: form.status })
      setEditing(null)
    } catch (err) {
  ...
  return (
    <div>
      <h3 className="font-bold text-slate-900 dark:text-slate-100">Table Data Editor</h3>
      <div className="mt-4 flex gap-3">
        <button onClick={addIndicator} className="px-3 py-2 bg-primary-600 text-white rounded">Tambah Indikator</button>
        <input ref={importRef} type="file" accept=".xlsx,.xls" onChange={importExcel} className="hidden" />
        <button onClick={() => importRef.current && importRef.current.click()} className="px-3 py-2 border rounded dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-700">Impor Excel</button>
        <button onClick={exportExcel} className="px-3 py-2 bg-slate-700 text-white rounded">Export Excel</button>
        <button onClick={exportPDF} className="px-3 py-2 bg-slate-700 text-white rounded">Export PDF</button>
      </div>
