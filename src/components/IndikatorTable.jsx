...
export default function IndikatorTable() {
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded shadow p-4 md:p-6">
      <h3 className="text-lg font-bold text-slate-900 dark:text-slate-100">GAMBARAN CAPAIAN INDIKATOR NASIONAL MUTU RUMAH SAKIT DI JAWA TIMUR TAHUN {year}</h3>
      <div className="mt-4 indikator-filter-group">
        <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
          <input
            ...
            className="px-2 py-1 border rounded text-sm w-full md:w-40 dark:bg-slate-900 dark:text-slate-100 dark:border-slate-700"
          />
          <select
            value={month}
            onChange={e => setMonth(e.target.value)}
            className="px-2 py-1 border rounded text-sm w-full md:w-24 dark:bg-slate-900 dark:text-slate-100 dark:border-slate-700"
          >
          ...
          <select
            ...
            className="px-2 py-1 border rounded text-sm w-full md:w-20 dark:bg-slate-900 dark:text-slate-100 dark:border-slate-700"
          >
          ...
          <button onClick={() => toggleSort('name')} className="px-2 py-1 bg-primary-50 dark:bg-slate-700 rounded text-sm w-full md:w-24">Sort Nama</button>
          <button onClick={() => toggleSort('capaian')} className="px-2 py-1 bg-primary-50 dark:bg-slate-700 rounded text-sm w-full md:w-24">Sort Capaian</button>
        </div>
      </div>
      ...
      <div className="mt-4 indikator-table-wrapper hidden md:block">
        <table className="w-full text-left">
          ...
          <tbody>
            {list.map(row => (
              <tr key={row.id} className="border-b dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-900">
      ...
      <div className="mt-4 indikator-card-list md:hidden">
        {list.map(row => (
          <div key={row.id} className="p-3 bg-white/60 dark:bg-slate-900/60 rounded border dark:border-slate-700">
